test_that("fortify_aggregate.epimodel_inner", {
    ...
    expect_true(all(rowSums(out[, 4:6]) == 1000))

    ############3
    out2 <- fortify_aggregate(EpiModel_agg_bd)
    expect_equal(colnames(out2), c("t", "orig_t", "sim", "X0", "X1", "X2"))
    ...
    expect_equal(colnames(out), c("t", "orig_t", "sim", "X0", "X1"))
    out2 <- fortify_aggregate(EpiModel_icm, states = c(s.num, i.num))
    expect_equal(colnames(out2), c("t", "orig_t", "sim", "X0", "X1"))
