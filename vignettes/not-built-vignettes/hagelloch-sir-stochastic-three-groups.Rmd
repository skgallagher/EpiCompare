...
agg <- grouped_agents %>% group_by(sim) %>%
...
The result of this is a prediction band that does capture the shape of the data.  In fact, it is even a worse approximation (by eye) than in the SIR group.  This is partially because we are looking at simulation error for a fixed estimate and not factoring in the uncertainty from the estimate itself. 
...
for(bb in 1:B){
  ...
  par_vals <-  c("par1" = par_val_mat[bb, 1],
                 ...
                 "par3" = par_val_mat[bb, 3],
                 "par4" = par_val_mat[bb, 4])
  par_vals <- ifelse(par_vals > 1, .999,
                     ifelse(par_vals < 0, .001, par_vals))  ## have to truncate
  ...
  agg_model <- grouped_agents %>% group_by(sim) %>%
     agents_to_aggregate(states = c(I, R)) 
  
  agg_model$sim <- bb
  agg_model$beta1 <- par_vals[1]
  agg_model$beta2 <- par_vals[2]
  agg_model$beta3 <- par_vals[3]
  agg_model$gamma <- par_vals[4]
...
This is perhaps equivalent to or moderately better than simulation produced by the SIR simulation that includes estimation and simulation error, but it is still clear that our model has difficulty in capturing the second peak in the data.
