...
Creating prediction regions and quantifying uncertainty in epidemiological function space is a complicated endeavor. In this vignette we focus on providing the reader some background for the approach presented in this package. Although our work was is based on work in [@Dalmasso2019]^[Ben is a co-author in that paper.] \cite*{Dalmasso2019},  we provide more statistical grounding for our shared approaches as well as smarter approaches for simulation based prediction regions for multivariate functional data (functions with output in multi-dimensional space). 
...
```{r warning = F, message = F}
psuedo_density <- distance_psuedo_density_function(tidy_dm, sigma)
...
```{r eval = T, cache = T}
quantile(dist_mat,probs = 0:10/10) %>%
  round(2) %>% t %>%
  data.frame(check.names = F) %>%
  DT::datatable(options = list(dom = 't'))

tau <- quantile(tidy_dm,probs = .30)["30%"]
