...
Using the same simulation parameters above we can simulate 10 epidemics:

```{r, eval = TRUE, fig.asp = .62, out.width = '60%'}
n_sims <- 10 # only code changed
n_time_steps <- 100
beta <- .1
gamma <- .03
init_SIR <- c(950, 50, 0)
...
We then use the `agents_to_aggregate_SIR_group` function to do the same thing:

```{r}
df_group <- out %>% group_by(sim) %>% agents_to_aggregate_SIR_group 
