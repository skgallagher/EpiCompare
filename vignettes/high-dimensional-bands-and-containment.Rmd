---
title: "High Dimensional Bands and Containment"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{High Dimensional Bands and Containment}
...
sim100 <- timeternR::simulate_SIR_agents(n_sims = n_sims,
...
df_group <- sim100 %>% group_by(sim) %>% 
  agents_to_aggregate(states = c("tI", "tR"), 
                      min_max_time = c(0,100)) %>% # REMOVE THIS LINE
  rename(S = "X0", I = "X1", R = "X2") 
```
...
b <- ggplot() +
  geom_confidence_band(data = df_group, 
                       ...
                       alpha_level = .9, cb_type = "delta_ball") +
  geom_path(data = df_single, aes(x = S, y = I, z = R), color = "blue") +
  coord_tern()
```
...
a <- ggplot() +
  geom_confidence_band(data = df_group, 
                       aes(x = S, y = I, z = R, 
                           sim_group = as.numeric(sim)),
                       alpha_level = .1, cb_type = "delta_ball") +
  geom_path(data = df_single, aes(x = S, y = I, z = R), color = "blue") +
  coord_tern()

testthat::expect_equal(a,b) # this is really bad (shouldn't be equal)...
```
...
out2 <- timeternR::simulate_SIR_agents(n_sims = 100,
                                      n_time_steps = 100,
                                      beta = beta, gamma = gamma,
                                      init_SIR = c(950, 50, 0))
...
ggplot() +
  geom_confidence_band(data = df_group, 
                       ...
                       fill = "blue", alpha = .5,
                       alpha_level = .1, cb_type = "delta_ball") +
  geom_confidence_band(data = df_group2, 
                       aes(x = S, y = I, z = R, 
                           sim_group = as.numeric(sim)),
                       color = "red",
                       fill = "red", alpha = .5,
...
ggplot() +
  geom_confidence_band(data = df_all, 
