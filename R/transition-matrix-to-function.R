...
#' @details This function dynamically creates a list of functions that turn the character transition formulas provided by the user in D_mat into \eqn{K^2} functions which take in arguments X0, ..., X{K-1)}, par1, par2, ..., par{n_pars}, and N and output the numeric transition rate of an individual currently in state \eqn{i} to move to state \eqn{j}.  
trans_mat_to_D_fxn <- function(D_mat, K,
                               n_pars){
    sym_list <- vector(mode = "list", length = K^2)
    my_args <- vector(mode = "list", length = K + n_pars + 1)
