...
\title{EpiCompare: A tidy \proglang{R} package for comparing and assessing epidemics and epidimology models}
\Plaintitle{EpiCompare: A tidy R package for comparing and assessing epidemics and epidimology models}
\Shorttitle{EpiCompare: an \proglang{R} package for comparing and assessing epidemics and models}
...
The recent (and currently on-going) COVID-19 global epidemic has galvenized public interest in understanding more about infectious disease modeling highlighted the usefulness of research in the area \cite{FILL IN}. Infectious disease models models attempt, typically, to 1) predict the spread of current and future epidemics such as the Center of Disease Control and Prevention's (CDCP) yearly ``Predict the Influenza Season Challenge'' \citep{biggerstaff2016}; or 2) learn about past and current epidemics to increase scientific knowledge about the mechanisms behind the spead of disease such as the effect of seasonality in the measles model of \cite{roberts2004}. The COVID-19 epidemic highlights how both goals are very important, and that preventing and mitigating the future spread of infectious disease which inflicts enormous burdens on the world: millions of lives lost and billions of dollars spent yearly, is so important. 


The current epidemic has also highlighted that these models are just one piece of the overall analysis pipeline. John Hopkin's \_\_\_ Dashboard and county, state and national tabulation of rudimentary numerical summaries emphasis the need to strong data to assess the spread of infectious disease (multiple \cite{FILL IN}?). Stilly, rightly so, a large amount of theoretical work goes into modeling epidemics, with different models focusing at the individal / agent level, network structure or just aggregate flows (review paper \cite{FILL IN}?). All placing individuals / proportions of the populations into different states (e.g. suspectible, exposed, infected, recovered, etc.). With all these models, review and comparison papers in the literature and through MIDAS (Models of Infectious Disease Agent Study) Control Center helps the individual practioner decide the correct approach.
...
\begin{figure}[t!]
\centering
\includegraphics{article-woven-visualization}
...
\pkg{MASS}~7.3.51.5 package. \proglang{R} itself
