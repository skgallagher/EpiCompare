...
step in the process (multiple \cite{}?).\} Still, rightly so, a large
amount of theoretical work goes into modeling epidemics, with different
models focusing at the individual / agent level, network structure or
just aggregate flows (review paper \cite{}?). All placing individuals /
...
One issue that the COVID-19 outbreak has brought to the forefront is the
importance of figures and graphs to inform, support, and even persuade
lay people of technical and scientific results \cite{}. Quantities and
trends shown in these visualizations include exponential growth curves,
S-curves, peak day and percent of infectious indviduals, ICU capacity,
and deaths over time and uncertainty of such estimates.

\pkg{EpiCompare} supports traditional visualizations (e.g.~\# in
condition vs.~time) and also introduces a novel visualization of
epidemics via time free analysis. In particular, we use ternary plots
that show Susceptible-Infectious-Recovered data/models (SIR). The
advantages of these ternary plots are 1) a standard way to compare
epidemics with different outbreak times and time scales, 2) a novel
interpretation of \(R_0\), the initial reproduction number under some
circumstances, and 3) clear visualization of joint SIR uncertainty via
conformal bands.

Ternary plots are sometimes used in chemistry \citep{} but only rarely
seen in the field of infectious disease epidemiology (to our knowledge,
only seen in \cite{}). Ternary plots illustrate the relationship of
constrained 3D data, namely of points
...
in examples of recent published works in \cite{}), and also happens to
be of the situational format required for ternary plots where
...
Ternary plots are time-free in the sense that the temporal scale does
not explicitly apear in the visualization and as a consequence, allow
for direct comparisons of outbreaks on different time scales (e.g.~days
vs.~years) or of SIR data of the same disease in different areas (our
first case study in Sec. \ref{sec:ex1} focuses on this).

Ternary plots also, under certain conditions, have a special
relationship with \(R_0\), the initial reproduction number, which has
been called the ``most important quantity in epidemiology'\,' \citep{}.
The reproduction is itself a time-free concept and is the average number
of secondary infections primary infection will generate when the primary
infection is added to a completely susceptible population. To see the
importance of \(R_0\), one need only read the newspaper \citep{} or look
at the length of the table of estimated \(R_0\) quantities for COVID-19
\citep{}.

In regards to traditional \(X\) vs.~time plots, \(R_0\) is difficult to
visualize. For example, as seen in Fig. \ref{fig:r0-trad}, it is
possible that two models have the same value of \(R_0\) but there is no
way of knowing that from looking at the graph.

It is possible to identify, at a glance, which epidemic paths have the
same value of \(R_0\) via ternary plot visualizations, under some
circumstances. To show, this we first need to introduce the classic,
deterministic Kermack and McKendrick SIR model where the initial number
in each state \((S(0), I(0), R(0))\) are known and \(N\) the population
size is constant. The movement of individuals among the states is given
by the following differential equations (Eq. \eqref{eq:sir-ode}) where
...
result from \cite{} and is shown in detail in Appendix \ref{app:proof}.
The consequence of Theorem \ref{thm:sir-scale} is that for two SIR
models that have the same initial percent of individuals in each state
and \(R_0\) then for every point on the epidemic path of the first SIR
model is also a point on the epidemic path of the second SIR model.
