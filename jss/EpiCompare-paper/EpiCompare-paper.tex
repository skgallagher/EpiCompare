...
\usepackage{amsmath} \usepackage{amsthm} \usepackage{afterpage} \usepackage[normalem]{ulem}
...
\sout{time} \textcolor{orange}{populations} and (2) by treating full
epidemics as
filaments\sout{ not functions (breaking away from a time-centric view)}
...
\textcolor{orange}{Mathematically}, epidemics are complex objects. They
are hard to assess and compare to one another due to the differences in
the diseases, the location where the outbreak occurs, how the
\textcolor{orange}{a?}ffected population reacts, and the timefeatures
(including start of the epidemic, speed of infection and more).
Time-invariant analysis \sout{attempts to make}
\textcolor{orange}{makes} different epidemics easier to compare by
removing many time dependent aspects of an epidemic. Instead
\textcolor{orange}{it} focuses on the overall \sout{"life"}
\textcolor{orange}{trajectory} of an epidemic, emphasizing the number of
lives affected.
\textcolor{orange}{really like the lives affected part; I think that's a powerful statement}
...
of certain epidemiological parameters, including the reproductive number
\(R_0\), which \citet{Gallagher2020} show can be \sout{heavily impacted}
\textcolor{orange}{strongly effected} by choices of beginning and ending
time points of an epidemic.
...
Susceptible-Infectious-Recovered (SIR) model \citep{hethcote2001}. For a
demonstration of \(R_0\) getting lost in a time-dependent analysis we
introduce the \citet{Kermack1927}'s common SIR model. This model
captures the transitions from one state to the next as a system of
...
\subsection[visualizing r0]{Visualizing \(R_0\)}\label{vis-r0}

With regards to traditional epidemic \(state\) vs.~\(time\) plots, when
...
clearer that Model 2 is following the same trajectory as Model 1 but is
not as far along in the infection process. We can see there is something
fundamentally linking these two different epidemics, and this
fundamental link turns out to be \(R_0\).

We can show this mathematically if we let our two epidemics be presented
...
\subsection[Beyond R0 and SIR]{Beyond \(R_0\) and Kermack's and
McKendrick SIR Models}\label{beyond-r0-sir}

Through \(R_0\) \textcolor{orange}{example}, we see that treating
epidemics more like a
...
\textcolor{orange}{filaments embedded in a lower dimensional space},
then we can better compare the overall structure of the epidemic and
...
\textcolor{violet}{In \pkg{EpiCompare}, w}e focus on understanding the
overall impact of an outbreak at the aggregate/population level,
\textcolor{violet}{which allows for examination of overall trends of an epidemic more easily}.
...
that account for subpopulations. In \pkg{EpiCompare}, we facilitate
subpopulation analysis by combining \pkg{dplyr}::\texttt{group\_by} and
\texttt{agent\_to\_aggregate} to provide aggregation at a group level.

\textcolor{violet}{[Ben says: this whole paragraph is new]} The
\texttt{agents\_to\_aggregate} function is flexible and can deal with a
wide range of information about each individual. It can, theoretically,
account for infinitely many states. This functionality allows the
practitioner to aggregate information relative to the standard states
(e.g.~``Susceptible'', ``Infectious'', and ``Recovered'') as well as add
states (e.g.~``Exposed'', ``iMmune'', ``Hospitalized'') (CITE).
...
\citep{Wickham2016, Hamilton2018}. \textcolor{orange}{come back?}
...
\textcolor{orange}{make sure SIR is defined before}
...
\texttt{agents\_to\_aggregate()}
\textbackslash ben\{\sout{with \pkg{ggplot2}'s} \texttt{geom\_path()}
\textcolor{violet}{\sout{and \pkg{ggtern}'s}} \texttt{coord\_tern()}
