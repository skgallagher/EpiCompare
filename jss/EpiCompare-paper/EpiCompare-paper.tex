...
\sout{typically} incorporate \pkg{EpiCompare} into any step in the data
analysis \sout{process}\textcolor{orange}{pipeline}, there are two
primary points of entry. The first point of entry is the very beginning
with pre-processing and visualizing raw data, and the second point of
entry is after modeling and simulation. Figure \ref{fig:pipeline2}
captures these different paths, and we
...
\textcolor{orange}{to a format that is friendly for both computers and programmers}
so it can be explored.
\sout{There are multiple ways to collect epidemiological data.}
\textcolor{orange}{Epidemiological data are collected in many different formats.}
...
individuals in each epidemic state. In fact,
\sout{usually}\textcolor{orange}{many times} only the number of new
infections at each time step (e.g.~weekly case counts) is observed.
Compartment totals (amounts of individuals in each state) are then
imputed from those case counts\sout{ along with}\textcolor{orange}{and}
...
outbreak at the aggregate/population level, which allows for
\textcolor{orange}{streamlined} examination of overall trends of an
epidemic.\sout{ more easily.}

\sout{In order t},To help the practitioner examine epidemics from an
aggregate/ population lens, we provide a function called
...
where grouping agents into subpopulations
(e.g.~sub\sout{populations}\textcolor{orange}{ groups} defined by age or
sex) can highlight different aggregate level trends. For example,
research by \citet{rvachev1985,anderson1992,worby2015} develop
state-based models that account for
\textcolor{orange}{differing disease dynamics in different}
subpopulations. In \pkg{EpiCompare}, we facilitate subpopulation
analysis by combining \textcolor{orange}{the function}
\pkg{dplyr}::\texttt{group\_by()} and \texttt{agent\_to\_aggregate()} to
provide aggregation \sout{at a}\textcolor{orange}{by} group level.

The \texttt{agents\_to\_aggregate()} function is flexible and can deal
with a wide range of information about each individual. \sout{It can,}
Theoretically, account for infinitely many states. This functionality
allows the practitioner to aggregate information relative to
\sout{the standard}\textcolor{orange}{common} states
(e.g.~``Susceptible'', ``Infectious'', and ``Recovered'') as well as
\sout{add}\textcolor{orange}{more complex} states (e.g.~``Exposed'',
``iMmune'', ``Hospitalized'')
(CITE\footnote{\textcolor{violet}{[Ben says: this was originally to back up the claim that SIR is the "standard" states - now we might need to have a paper that says that SIR is the standard states and also suggests other states...]}\textcolor{orange}{Shannon says: I changed 'standard' to 'common' so maybe that's a less loaded word.  Otherwise I think I'd just cite anderson and may 1992}}).
Additionally, \texttt{agents\_to\_aggregate()} \sout{also} permits
indicators for death/exit and birth/entry dates. Overall, this function
is a powerful tool for pre-processing data,
\textcolor{orange}{and it lowers the barrier for entry into data analysis for less experienced practicioners.}\footnote{\textcolor{orange}{Shannon says: I added this in theme with 'highlighting a point of entry'}}

\textbf{EDA}

\sout{With raw data, }``Getting to know'' \sout{our} data currently
means figuring out \sout{good}\textcolor{orange}{useful} combinations of
visualizations, numerical summaries and
\sout{subsets}\textcolor{orange}{groupings}. An expert coder has many
ways to successfully explore the data in an aggregate lens using
\texttt{agents\_to\_aggregate()}.
\textcolor{orange}{For less experienced coders,} \pkg{EpiCompare} also
includes tools to rapidly explore data that has three epidemic states.
Building on the tools in \pkg{ggplot2} and \pkg{ggtern},
\sout{our}\pkg{EpiCompare}`s \texttt{geom\_aggregate()} provides a rapid
way to explore different subpopulations' experiences of an epidemic
\citep{Wickham2016, Hamilton2018}. \textbackslash sout\{By combining the
ideas behind agents\_to\_aggregate for three-state models to examine
subpopulation trajectories in a 3d simplex space\}
\citep{Wickham2016, Hamilton2018}\footnote{\textcolor{orange}{I got rid of the end of this sentence because I think it was putting multiple ideas in one sentence.}}.
...
models and (2) our \textcolor{orange}{3d} simplex representation of
...
. The special classes often contain a plethora of information from
residuals, model diagnostics, input parameters, and more. While
incredibly useful, these special classes can be difficult for novice
coders to navigate.{]}
...
To this end, \pkg{EpiCompare} \textcolor{violet}{provides\sout{adapted}}
a series of \texttt{fortify}-style
...
\textcolor{orange}{infectious disease modeling and simulation} packages
like \pkg{pomp} and \pkg{EpiModel} into tidy-styled data frames which
contain information about the total number of individuals in each state
at a given time, for a given simulation. These fortify functions have
output that is consistent with that of \texttt{agents\_to\_aggregate()}.

\textcolor{violet}{[Ben says: new paragraph]} To utilize simulations of
...
Specifically, we provide the function \texttt{filament\_compression()}
to convert simulation(s) to filaments as expressed by presenting the
...
state
space\footnote{\textcolor{violet}{[Ben: this should be cleaned up.  I also 'highlighted' why this makes epicompare a good entry point]}}

\textcolor{orange}{Shannon's attempt at above paragraph is below..  I tried to more smoothly connect to previous paragraph and then kinda copped out at trying to reword the filament description by just referring back to the previous section.}

{[}\pkg{EpiCompare} also provides a tool to convert time-dependent
epidemic simulations into their time-invariant filamental
representations. Specifically, \texttt{filament\_compression()} converts
simulations to filaments (see Section \ref{sec:time-invariant}){]} so
practitioners can view model simulations and results through a
time-invariant lens. These tools were developed to provide another
natural entry point into the \pkg{EpiCompare} data analysis pipeline for
situations where modeling and analysis is already completed and
practitioners are looking for simple and transparent tools to help
understand and disseminate results.{]}
...
\textcolor{orange}{I don't think so, best left to the vignette and data example}
...
\textcolor{violet}{[Ben says: should we define these more in this paragraph? Introducted in 2.2. as well and not defined.]}\textcolor{orange}{[I'd throw a citation or two but again limit the detail.]}
