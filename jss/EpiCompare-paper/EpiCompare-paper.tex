...
\footnote{\textcolor{violet}{[Ben says: I'd like to replace these two sentences will a new one. The original goal was to make a strong connection between the first sentence "There are..." to the previous sentence, but this lead to a rewrite (which I know isn't best). Suggestion: "Researchers, including \citet{rvachev1985,anderson1992,worby2015}, often are interesting in more granular trends that can be detected by aggregati\textcolor{orange}{on}, conditional on subpopulations (e.g. subpopulations defined by age or sex).]}\textcolor{orange}{I like your edits}}There
are often situations where grouping agents into subpopulations
...
In \pkg{EpiCompare}, we facilitate subpopulation
analysis\footnote{\textcolor{orange}{Changed to try to clarify how how agents to aggregate works with group by and why: (split-apply-combine)}}
by extending \texttt{agents\_to\_aggregate()} to work with
\pkg{dplyr}::\texttt{group\_by()}.\sout{, the tidyverse approach to the split-apply-combine paradigm for data analysis}
\citep{wickham2011}.

{[}{[}\textcolor{red}{OLD SENTENCE} by combining the function
\pkg{dplyr}::\texttt{group\_by()} and \texttt{agents\_to\_aggregate()}
to provide
\textcolor{violet}{group level aggregation\sout{aggregation at a subpopulation level}}\footnote{\textcolor{violet}{[Ben says: This sentence seems a bit repetitive with "supopulation". ]}\textcolor{orange}{agreed. aggregation at a group level} \textcolor{violet}{[Ben says: I think the replacement doesn't clean things up - I probably wasn't clear that I actually don't think split-apply-combine is well known. My edit to the old sentence was aimed to making it clear the process through our words "group-then-aggregate".]}}.{]}{]}
...
models were developed because \textcolor{violet}{\sout{(1)}} SIR models
are some of the most common and basic epidemic state-based models and
\textcolor{violet}{\sout{(2)}}\footnote{\textcolor{violet}{[Ben says: I don't think this is necessary anymore, reads more clearly without these.]}\textcolor{orange}{accepted}}
our three-dimensional simplex representation of these epidemics
emphasizes a time-invariant representation of the data (for a refresher
...
and/or simulation.
\textcolor{violet}{\sout{In this step and the next step (post-processing), we highlight how practitioners can pair models and simulations of epidemics from outside of \pkg{EpiCompare} with analysis and simulation tools in \pkg{EpiCompare}.}}\footnote{\textcolor{violet}{[Ben says: I think we should drop this as it's not really true about this section and tries to weirdly introduce the next section.]}\textcolor{orange}{accept}}
While \pkg{EpiCompare} does not focus on fitting models to data, we do
provide some flexible functions for simulation of basic discrete-time
epidemic-state models. These functions simulate individual-level
information based on practitioner estimated transition rates between
states and can be combined with \texttt{agents\_to\_aggregate()} to view
these simulations through an aggregate lens. The function
...
\textcolor{violet}{\sout{This function allows for easy access to SIR model analysis and comparison.}}\footnote{\textcolor{violet}{[Ben says: see edit below as well. This sentence seemed misplaced, and we have a very similar sentence a few lines later. I'm happy to be overruled here.]}\textcolor{orange}{accept, yeah it's unnecessary}}
Beyond SIR models, the function \texttt{simulate\_agents()} takes as
input a user-specified state-transition matrix and other epidemic
parameters to allow the user to create simulations for an outbreak with
\textit{any} number of states and any number of transitions among them.
This flexibility in states can be used to also reflect group-based
dynamics. Both of these functions allow users to explore the space of
models in an intuitive way without getting bogged down by too much
mathematical detail. For consistency, we have made output from
...
to disseminate the results in an easily digestible format.
\textcolor{violet}{\sout{In general, post-processing of modeling and simulation consists of making summary statistics, plots, and tables.}}\footnote{\textcolor{violet}{[Ben says: this really isn't true anymore - that's more the next section.]}\textcolor{orange}{accept}}
In \pkg{EpiCompare}, we provide (1) functions to standardize simulation
and model \textcolor{orange}{objects}~sout\{output\}
\footnote{\textcolor{violet}{[Ben says: I find "output" a weird word - but my only suggested replacment is "objects", which isn't much better and is used in the next paragraph...]}\textcolor{orange}{trying out objects..}}
from external packages and (2) a function to transform standardized
simulation and model output into a format amenable to time-invariant
analysis.\footnote{\textcolor{orange}{new signpost.  There are two paragraphs below corresponding to the two points.  The second paragraph could use another sentence maybe.  I tried to emphasize the process of output -> standard format -> transformation to time invariant analysis.}}

Modeling and simulation \textcolor{orange}{objects} \sout{output} can be
very complex objects, and as a result, a number of epidemic modeling
...
classes can be difficult for novice coders to handle. To this end,
\pkg{EpiCompare} provides a series of fortify-style methods, called
\code{fortify_aggregate()} which transform output from infectious
disease modeling and simulation packages like \pkg{pomp} and
\pkg{EpiModel} into tidy-styled data frames which contain information
about the total number of individuals in each state at a given time, for
a given simulation. These fortify functions have output that is
consistent with that of \code{agents_to_aggregate()}. These standardized
outputs can then be piped to summaries, tables, and plots.

Because
\textcolor{violet}{epidemic data is stored in a temporal way, \sout{the fortified output contains temporal information and}}\footnote{\textcolor{violet}{[Ben says: this isn't my best suggestion, but I think we should avoid suggesting filament\_compression is only for the fortified data - it can be used on processed raw data as well.]} \textcolor{orange}{accept}}
we provide the function, \code{filament_compression()}, to transform
...
epidemic modeling \proglang{R} packages return a special class. The
...
of time-invariant
analysis.\footnote{\textcolor{orange}{a bit of an awk transition but I wanted to use past wording about TIA.}}
...
Table \ref{tab:hags-people}. With \pkg{EpiCompare}, we can easily
