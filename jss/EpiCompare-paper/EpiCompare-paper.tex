...
observed. \textcolor{violet}{In this setting, c}ompartment totals
(amounts of individuals in each state) are then imputed from those case
counts \textcolor{violet}{and using} other information about the disease
and the population of
interest\footnote{\textcolor{violet}{[Ben says: updates to connect to the previous sentence more clearly and the second was grammar.]}}.
In \pkg{EpiCompare}, we focus on understanding the overall impact of an
...
\footnote{\textcolor{violet}{[Ben says: I'd like to replace these two sentences will a new one. The original goal was to make a strong connection between the first sentence "There are..." to the previous sentence, but this lead to a rewrite (which I know isn't best). Suggestion: "Researchers, including \citet{rvachev1985,anderson1992,worby2015}, often are interesting in more granular trends that can be detected by aggregating conditional on subpopulations (e.g. subpopulations defined by age or sex).]}}There
...
\pkg{dplyr}::\texttt{group\_by()}, the tidyverse approach to the
split-apply-combine paradigm for data analysis \citep{wickham2011}.
...
\textcolor{violet}{\sout{(2)}}\footnote{\textcolor{violet}{[Ben says: I don't think this is necessary anymore, reads more clearly without these.]}}
...
\textcolor{violet}{\sout{This function allows for easy access to SIR model analysis and comparison.}}\footnote{\textcolor{violet}{[Ben says: see edit below as well. This sentence seemed misplaced, and we have a very similar sentence a few lines later. I'm happy to be overruled here.]}}
...
dynamics.
\textcolor{violet}{\sout{In turn, this allows for} Both of these functions allow}
...
\textcolor{violet}{\sout{In general, post-processing of modeling and simulation consists of making summary statistics, plots, and tables.}}\footnote{\textcolor{violet}{[Ben says: this really isn't true anymore - that's more the next section.]}}
In \pkg{EpiCompare}, we provide (1) functions to standardize simulation
and model
output\footnote{\textcolor{violet}{[Ben says: I find "output" a weird word - but my only suggested replacment is "objects", which isn't much better and is used in the next paragraph...]}}
