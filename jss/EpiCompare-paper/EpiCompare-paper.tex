...
subpopulations by using the \pkg{tidyverse} paradigm to first
\texttt{group\_by()}\footnote{The \texttt{group\_by} function is from
  the \pkg{dplyr} package.} the subpopulation defining features and then
...
successfully do this in many ways, but \pkg{EpiCompare} also includes
...
groups epidemic trajectory in 3d simplex space using \pkg{ggplot2} and
\pkg{ggtern} \citep{Wickham2016, Hamilton2018}. Visualization tools for
