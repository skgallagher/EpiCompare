...
SHANNON: I FEEL LIKE WE CAN JUST CLEAN UP TO ORIGINAL SECTION AND PUT IT BACK IN HERE
...
```
The result of our simulation is the object \code{agents} which is a `r nrow(agents)` $\times$ `r ncol(agents)` which details the time of entry into the $S$, $I$, and $R$ states for a given simulation.  Before we examine the results of this simple SIR model, we will also examine another, more sophisticated SIR model, this time from the package \pkg{EpiModel}.  Briefly, this model fits a contact network, where the class of the student is a covariate and then simulates a SIR-epidemic on that network.
