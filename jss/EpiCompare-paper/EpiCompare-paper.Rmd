...
In this section, we present the tools implemented in \pkg{EpiCompare} and explain how they aid in the  data analysis pipeline.  In Fig. \ref{fig:pipeline2}, we illustrate how our package's functions fit into the data analysis pipeline introduced in Fig. \ref{fig:pipeline}.  All front-facing functions are aimed to be as user-friendly as possible. We also focus on providing the user "tidyverse" style functions, that encourage piping objects from one function to the next and follow clear "verb" naming schemes \citep{Wickham2019}. Although users can \sout{typically} incorporate \pkg{EpiCompare} into any step in the data analysis \sout{process}\shannon{pipeline}, there are two primary points of entry.  The first point of entry is the very beginning with pre-processing and visualizing raw data, and the second point of entry is after modeling and simulation. Figure \ref{fig:pipeline2} captures these different paths, and we highlight\footnote{\ben{[Ben says: we need to make sure we actually do highlight ]}} both approaches and how to leverage \pkg{EpiCompare} in the subsections below.


**Data Pre-processing**

The first step of most data analysis is "cleaning" the data \shannon{to a format that is friendly for both computers and programmers} so it can be explored. \sout{There are multiple ways to collect epidemiological data.} \shannon{Epidemiological data are collected in many different formats.} Sometimes individual records are collected, with times of different states of the epidemic (infection, recovery, etc.) as well as individual information like network structure, location, and sub-population information. Other data collections focus on aggregate counts of individuals in each epidemic state.  In fact, \sout{usually}\shannon{many times} only the number of new infections at each time step (e.g. weekly case counts) is observed.  Compartment totals (amounts of individuals in each state) are then imputed from those case counts\sout{ along with}\shannon{and} other information about the disease and the population of interest.  In \pkg{EpiCompare}, we focus on understanding the overall impact of an outbreak at the aggregate/population level, which allows for \shannon{streamlined} examination of overall trends of an epidemic.\sout{ more easily.}  

\sout{In order t},To help the practitioner examine epidemics from an aggregate/ population lens, we provide a function called `agents_to_aggregate()`. This function transforms data about individual/agents' initial entry into each state (e.g. start of infection, start of recovery, etc.) to an aggregate view of how many individuals were in a state at a given time. There are often situations where grouping agents into subpopulations (e.g. sub\sout{populations}\shannon{ groups} defined by age or sex) can highlight different aggregate level trends. For example, research by \citet{rvachev1985,anderson1992,worby2015} develop state-based models that account for \shannon{differing disease dynamics in different} subpopulations. In \pkg{EpiCompare}, we facilitate subpopulation analysis by combining \shannon{the function} \pkg{dplyr}::`group_by()` and `agent_to_aggregate()` to provide aggregation \sout{at a}\shannon{by} group level.

The `agents_to_aggregate()` function is flexible and can deal with a wide range of information about each individual. \sout{It can,} Theoretically, \shannon{this `agents_to_aggregate()` can} account for infinitely many states.  This functionality allows the practitioner to aggregate information relative to \sout{the standard}\shannon{common} states (e.g. "Susceptible", "Infectious", and "Recovered") as well as \sout{add}\shannon{more complex} states (e.g. "Exposed", "iMmune", "Hospitalized") (CITE\footnote{\ben{[Ben says: this was originally to back up the claim that SIR is the "standard" states - now we might need to have a paper that says that SIR is the standard states and also suggests other states...]}\shannon{Shannon says: I changed 'standard' to 'common' so maybe that's a less loaded word.  Otherwise I think I'd just cite anderson and may 1992}}). Additionally, `agents_to_aggregate()` \sout{also} permits indicators for death/exit and birth/entry dates. Overall, this function is a powerful tool for pre-processing data, \shannon{and it lowers the barrier for entry into data analysis for less experienced practicioners.}\footnote{\shannon{Shannon says: I added this in theme with 'highlighting a point of entry'}}

**EDA**

\sout{With raw data, }"Getting to know" \sout{our} data currently means figuring out \sout{good}\shannon{useful} combinations of visualizations, numerical summaries and \sout{subsets}\shannon{groupings}. An expert coder has many ways to successfully explore the data in an aggregate lens using `agents_to_aggregate()`. \shannon{For less experienced coders,} \pkg{EpiCompare} also includes tools to rapidly explore data that has three epidemic states. Building on the tools in \pkg{ggplot2} and \pkg{ggtern}, \sout{our}\pkg{EpiCompare}'s `geom_aggregate()` provides a rapid way to explore different subpopulations' experiences of an epidemic \citep{Wickham2016, Hamilton2018}. \sout{By combining the ideas behind agents_to_aggregate for three-state models to examine subpopulation trajectories in a 3d simplex space} \citep{Wickham2016, Hamilton2018}\footnote{\shannon{I got rid of the end of this sentence because I think it was putting multiple ideas in one sentence.}}.  Visualization tools for three-state models were developed because (1) SIR models are some of the most common and basic epidemic state-based models and (2) our \shannon{3d} simplex representation of these epidemics emphasizes a "time-invarance" representation of the data (for a refresher see Section \ref{sec:time-invariant}).
...
\ben{After getting a good sense of what a past or current epidemic looks like with EDA, the next step is often model fitting and/or simulations. In this step and the next step (post-processing), we discuss how to \shannon{easily include external models and simulations originating outside into the \pkg{EpiCompare} data analysis pipeline.} \sout{include practioners' use of tools to create models and simulations that exist outside of \pkg{EpiCompare}.} \shannon{While \pkg{EpiCompare}}\sout{This package }does not focus on fitting model(s) to data, we do provide some flexible functions for simulation of basic discrete-time epidemic-state models. \ben{After estimating or predicting transition rates between states,} these functions produce individual-level \ben{simulations\sout{information}} and can be \ben{\sout{naturally}} combined with `agents_to_aggregate()` to view these simulations through an aggregate lens.  The function `simulate_SIR_agents()` simulates a \shannon{basic} SIR epidemic with user inputs for the number of simulations, the initial number in each state, the infection and recovery parameters $(\beta, \gamma$), and the total number of discrete time steps.  This function allows for easy access to SIR model analysis and comparison.  Beyond SIR models, the function `simulate_agents()` takes as input a user-specified \shannon{state-}transition matrix and other epidemic parameters to allow the user to create simulations for an outbreak with \textit{any} number of states and any number of transitions among them.  This flexibility in states can be used to also reflect group-based dynamics.  \shannon{In turn, t}his allows for users to explore the space of models in an intuitive way without getting bogged down by too much mathematical detail. For consistency, we have made output from `simulate_agents()` and `simulate_SIR_agents()` compatible with `agents_to_aggregate()` so aggregate information may easily be accessed.
...
\ben{If the practitioner wishes to compare \shannon{models-to-observations or even models-to-models }\sout{between models and with their true epidemics,} she needs to \shannon{post}-process her models and simulations}\shannon{to disseminate the results in an easily digestable format.} \ben{In general, [p}ost-processing of modeling and simulation consists of making summary statistics, plots, tables\sout{, and other ways to disseminate information to the public.]}  \ben{The summaries can be very complex, and a}[s a result, a number of epidemic modeling \ben{\proglang{R}} packages return a special class\sout{}, specific to their modeling}.  The special classes often contain a plethora of information from residuals, model diagnostics, input parameters, and more.  While incredibly useful, these special classes can be difficult for novice coders to navigate.]
...
To this end, \pkg{EpiCompare} \ben{provides\sout{adapted}} a series of `fortify`-style \ben{methods\sout{functions}}\footnote{\ben{[Ben says: Not tied to this change.]}}, called `fortify_aggregate()` which transform output from \shannon{infectious disease modeling and simulation} packages like \pkg{pomp} and \pkg{EpiModel} into tidy-styled data frames which contain information about the total number of individuals in each state at a given time, for a given simulation.  These fortify functions have output that is consistent with that of `agents_to_aggregate()`.

\shannon{[Shannon says: Here Ben talks about filament compression and tidy_dist mats, etc.]} \ben{[Ben says: not currently thinking about including tidy_dist stuff - will need to think about this as it is inlcudding in the pipeline 2 image...]}

\ben{[Ben says: new paragraph]} To utilize simulations of epidemics in later time-invariant analysis we also provide a function to convert temporally defined epidemics to filamental representations. Specifically, we provide the function `filament_compression()` to convert simulation(s) to filaments as expressed by presenting the epidemic as a ordering of some common fixed number of points so that they are equally spaced along the original path in the proportional state space\footnote{\ben{[Ben: this should be cleaned up.  I also 'highlighted' why this makes epicompare a good entry point]}}

\shannon{Shannon's attempt at above paragraph is below..  I tried to more smoothly connect to previous paragraph and then kinda copped out at trying to reword the filament description by just referring back to the previous section.}
[\pkg{EpiCompare} also provides a tool to convert time-dependent epidemic simulations into their time-invariant filamental representations.  Specifically, `filament_compression()` converts simulations to filaments (see Section \ref{sec:time-invariant})] so practitioners can view model simulations and results through a time-invariant lens.  These tools were developed to provide another natural entry point into the \pkg{EpiCompare} data analysis pipeline for situations where modeling and analysis is already completed and practitioners are looking for simple and transparent tools to help understand and disseminate results.]
