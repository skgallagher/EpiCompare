...
The first step of most data analysis is cleaning the data \ben{so it can \sout{to}} be explored. There are multiple ways to collect epidemiological data. Sometimes individual records \ben{are collected}, with times of different states of the epidemic (infection, recovery, etc.) as well as individual information like network structure, location, and sub-population information\ben{.\sout{will be collected, whereas}} \ben{O}ther data collections focus on aggregate counts of individuals in each epidemic state.  In fact, usually only the number of new infections at each time step (e.g. weekly case counts) is observed.  Compartment totals \ben{(amounts of individuals in each state)} are then imputed from those case counts along with other information about the disease and the population of interest.  \ben{In \pkg{EpiCompare}, w}e focus on understanding the overall impact of an outbreak at the aggregate/population level, \ben{which allows examination of overall trends of an epidemic more easily}.  
...
\ben{[Ben says: this is actually an older version:]} With raw data, "getting to know" our data currently means figuring out good combinations of visualizations, numerical summaries and subsets. An expert coder can start from `agents_to_aggregate` to successfully do this in many ways, but \pkg{EpiCompare} also includes tools to rapidly explore your data if your has at least three epidemic states (although these tools work best with exactly three epidemic states). Our `geom_aggregate` provides a rapid way to explore different subpopulations' experience of the epidemic, and combines the ideas behind `agents_to_aggregate` for the SIR case, with `geom_path` and `coord_tern` to visualize any number of groups epidemic trajectory in 3d simplex space using \pkg{ggplot2} and \pkg{ggtern} \citep{Wickham2016, Hamilton2018}. Visualization tools for SIR models were developed because (1) SIR models are some of the most common and basic epidemic state-based models and (2) our simplex representation of these epidemics emphasizes a "time-invarance" representation of the data (for a refresher see Section \ref{sec:time-invariant}).
