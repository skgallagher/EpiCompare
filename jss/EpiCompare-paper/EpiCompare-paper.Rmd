...
Finally, the data analysis pipeline often ends with plots, tables, and summary statistics that are used to compare different models, and simulations to one another. In \pkg{EpiCompare} we provide a set of comparison and assessment tools for model and simulation results that extend beyond the standard performance metrics (e.g. mean squared error or AIC) and into the lens of time-invariant analysis.\footnote{a bit of an awk transition but I wanted to use past wording about TIA.}  We have found these tools to be specifically applicable for situations where only one "season" or "cycle" of an epidemic has occurred.
