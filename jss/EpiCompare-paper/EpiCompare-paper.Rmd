...
\pkg{EpiCompare} delivers \textit{time-invariance} centric analysis by (1) taking a global, not marginal view of how epidemics move through \sout{time} \shannon{populations} and (2) by treating full epidemics as filaments\sout{ not functions (breaking away from a time-centric view)} and not points produced by functions of time. The following section aims to highlight the strengths of \textit{time-invariance} and define the mathematical foundations that \pkg{EpiCompare}’s tools stand upon.

\shannon{Mathematically}, epidemics are complex objects.  They are hard to assess and compare to one another due to the differences in the diseases, the location where the outbreak occurs,  how the \shannon{a?}ffected population reacts, and the timefeatures (including start of the epidemic, speed of infection and more). Time-invariant analysis \sout{attempts to make} \shannon{makes} different epidemics easier to compare by removing many time dependent aspects of an epidemic. Instead \shannon{it} focuses on the overall \sout{"life"} \shannon{trajectory} of an epidemic, emphasizing the number of lives affected. \shannon{really like the lives affected part; I think that's a powerful statement}
