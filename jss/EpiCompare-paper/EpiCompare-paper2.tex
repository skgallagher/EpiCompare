...
process, as has been seen in the current COVID-19
\textcolor{orange}{\sout{epi}}\textcolor{orange}{pan}demic
\citep{dong2020,cdc-covid-tracker2021,wp-covid-tracker2021}.
\footnote{\textcolor{violet}{[Ben says: probably should have a conclusion sentence here - seems to end abruptly. *This is less so the case now.]}\textcolor{orange}{Seems fine to me now}}

With \textcolor{orange}{\sout{the}} many visualization and exploratory
tools, models and modeling paradigms, and reviews and comparisons in the
literature and through the MIDAS (Models of Infectious Disease Agent
Study) network \citep{midasnetwork2021} \textcolor{orange}{available},
this field has a \textcolor{orange}{\sout{lot of}}
\textcolor{orange}{number of} devices to \textcolor{orange}{\sout{aid}}
\textcolor{orange}{help} an individual practitioner decide the correct
approach. For example, \proglang{R} packages such as \pkg{surveillance},
...
times or time scales
\textcolor{orange}{in order}\footnote{\textcolor{orange}{wanted a clearer signal that this is a bridge to a new thought}}to
understand the processes at play. With time-invariant analysis,
comparing decades-long outbreaks of HIV in the US to a 10 day outbreak
of norovirus on a cruise ship is possible. Compared to time-dependent
comparison tools for state-space modeling, time-invariant analysis can
make it easier to compare state-space epidemic representations in a more
global, holistic fashion. Many time-dependent comparison tools only
examine the proportion of individuals in each state (at a given time) in
a piece-wise / marginal fashion. These time-dependent approaches can
reduce the amount of connections that can be seen and insights that can
be drawn\}, similar to examining projections of a multidimensional
distribution onto a single axis, one at a time. Tools in
\pkg{EpiCompare}
\textcolor{orange}{\sout{give the user the ability to extend their} \textcolor{orange}{extend the user}}
...
\textcolor{orange}{from start to finish of the data analysis pipeline}\footnote{\textcolor{orange}{maybe clearer? up to you}}
